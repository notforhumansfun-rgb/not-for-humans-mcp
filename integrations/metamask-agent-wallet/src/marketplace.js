...
function uint(value, label) {
  ...
  if (parsed > MAX_UINT256) fail(`${label} must fit in one ABI uint256 word.`);
  return parsed;
}

function pinnedContracts(target, chainId) {
  if (!target || typeof target !== 'object' || Array.isArray(target)
      || target.schema !== 'notforhumans-agent-wallet-target/1') {
    fail('a pinned target configuration is required.');
  }
  const expectedChainId = uint(target.chainId, 'pinned target chainId');
  if (chainId !== expectedChainId) fail('chainId does not match the pinned target.');
  return {
    marketplace: address(target.contracts?.marketplace, 'pinned target contracts.marketplace'),
    token: address(target.contracts?.token, 'pinned target contracts.token'),
    weth: address(target.contracts?.weth, 'pinned target contracts.weth'),
  };
}
...
export function createAutonomyRehearsalPlan(
  input,
  { now = Math.floor(Date.now() / 1000), target } = {},
) {
  ...
  if (chainId !== 11155111n) fail('V19 autonomy rehearsals are Sepolia-only.');
  ...
  for (const [name, actual] of Object.entries({ marketplace, token, weth })) {
    if (actual !== expectedContracts[name]) fail(`contracts.${name} does not match the pinned target.`);
  }
  ...
  if (expiry > BigInt(now + 7 * 24 * 60 * 60)) fail('expiry cannot exceed seven days in the V19 rehearsal.');
  ...
  const steps = [
    transaction({ signer: seller, role: 'seller', contract: token, functionName: 'approve', args: [marketplace, tokenId], intent: `Approve only NFH #${tokenId} for the V19 marketplace` }),
  ...
  return {
    ...
    contracts: { marketplace, token, weth },
    agents: {
...
export function createTransactionHandoffs(plan) {
  if (plan?.schema !== 'notforhumans-autonomy-rehearsal-plan/1' || !Array.isArray(plan.steps)) fail('validated V19 plan required.');
